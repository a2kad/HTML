...
/**
 * \file    test/phpunit/NewVisionFunctionalTest.php
 * \ingroup newvision
 ...
 *
 * @package Testnewvision
 */
class NewVisionFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modNewVision')->click();
		} else {
			// Disable the module
			$this->byHref('modNewVision')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/newvision/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('newvision/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/newvision/admin/about.php');
		$this->authenticate();
		return $this->assertContains('newvision/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('newvisionwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modNewVision_NewVisionTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modNewVision_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
